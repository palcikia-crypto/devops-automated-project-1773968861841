...
export async function generateAIResponse(messages: any[], retryCount = 0): Promise<any> {
  if (!ai && !process.env.OPENROUTER_API_KEY) throw new Error("API key is missing");
  ...
  try {
    if (ai) {
      ...
      return generateOpenRouterResponse(messages);
    }
  } catch (error: any) {
    // Handle 429 Resource Exhausted with exponential backoff or fallback to OpenRouter
